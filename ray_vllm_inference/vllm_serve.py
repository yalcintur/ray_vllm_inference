...
class VLLMGenerateDeployment:
    ...
    async def _stream_results_websocket(
        self, output_generator, websocket: WebSocket
    ) -> None:
        async for request_output in output_generator:
            output = request_output.outputs[0]
    ...
    async def ws_generate(self, websocket: WebSocket):
        ...
        try:
            while True:
                ...
                if request.prompt:
                    ...
                    if self.model_config:
                        ...
                        return create_error_response(
                            ...
                            'Parameter "messages" requires a model config',
                        )
                ...
                await self._stream_results_websocket(output_generator, websocket)
    ...
    async def generate(
        ...
        try:
            if not request.prompt and not request.messages:
                return create_error_response(
                    HTTPStatus.BAD_REQUEST, "Missing parameter 'prompt' or 'messages'"
                )
            ...
            if request.stream:
                ...
                return StreamingResponse(
                    self._stream_results(output_generator), background=background_tasks
                )
